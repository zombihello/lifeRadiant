...
/*
	Конвертировать вершины и индексы
	полигонов в формат lifeEngine (необходимые для VAO)
*/

void ConvertGeometry_lifeEngine()
{
	int					SizeBuffer_DrawVertexes = 0;
	int					NumberDrawVertexes = 0;
	int					NumberDrawIndexes = 0;

	bspDrawVert_t		*Buffer_Vertexes;
	int*				CountVertexInSurface;

	/* Выделяем память для массива количества вершин у каждой плоскости */
	CountVertexInSurface = ( int* ) malloc( numBSPDrawSurfaces * sizeof( *CountVertexInSurface ) );

	/* Подсчитываем размер буфера вершин */
	for ( int i = 0; i < numBSPDrawSurfaces; i++ )
	{
		CountVertexInSurface[ i ] = 0;

		for ( int j = 0; j < bspDrawSurfaces[ i ].numIndexes; j++ )
		{
			SizeBuffer_DrawVertexes++;
			CountVertexInSurface[ i ]++; // Считаем кол-во вершин для плоскости
		}
	}

	/* Выделяем память для буфера вершин */
	Buffer_Vertexes = ( bspDrawVert_t* ) malloc( SizeBuffer_DrawVertexes * sizeof( *Buffer_Vertexes ) );

	/* Записываем в буфер данные */
	for ( int i = 0, Id = 0; i < numBSPDrawSurfaces; i++ )
		for ( int j = bspDrawSurfaces[ i ].firstIndex, k = 0; k < bspDrawSurfaces[ i ].numIndexes; j++, k++, Id++ )
			Buffer_Vertexes[ Id ] = bspDrawVerts[ bspDrawSurfaces[ i ].firstVert + bspDrawIndexes[ j ] ];

	/* Освобождаем и выделяем память для нового массива вершин */
	free( bspDrawVerts );
	bspDrawVerts = ( bspDrawVert_t* ) malloc( SizeBuffer_DrawVertexes * sizeof( *bspDrawVerts ) );

	/* Разбиваем вершины на индексы */
	for ( int idSurface = 0, idVert = 0; idSurface < numBSPDrawSurfaces; idSurface++ )
	{
		...
		for ( int Count = 0; Count < CountVertexInSurface[ idSurface ] && idVert < SizeBuffer_DrawVertexes; Count++, idVert++ )
		{
			...
			/* Проходимся по массиву вершин и ищем совпадения */
			qboolean IsFind = qfalse;
			for ( int id = 0; id < NumberDrawVertexes; id++ )
			{
				bspDrawVert_t* Vertex = &bspDrawVerts[ id ];
				qboolean Compare_XYZ, Compare_ST, Compare_Lightmap, Compare_Normal, Compare_Color;

				Compare_XYZ = TempVertex.xyz[ 0 ] == Vertex->xyz[ 0 ] && TempVertex.xyz[ 1 ] == Vertex->xyz[ 1 ] && TempVertex.xyz[ 2 ] == Vertex->xyz[ 2 ];
				Compare_Normal = TempVertex.normal[ 0 ] == Vertex->normal[ 0 ] && TempVertex.normal[ 1 ] == Vertex->normal[ 1 ] && TempVertex.normal[ 2 ] == Vertex->normal[ 2 ];
				Compare_ST = TempVertex.st[ 0 ] == Vertex->st[ 0 ] && TempVertex.st[ 1 ] == Vertex->st[ 1 ];
				Compare_Lightmap = TempVertex.lightmap[ 0 ][ 0 ] == Vertex->lightmap[ 0 ][ 0 ] && TempVertex.lightmap[ 0 ][ 1 ] == Vertex->lightmap[ 0 ][ 1 ];
				Compare_Color = TempVertex.color[ 0 ][ 0 ] == Vertex->color[ 0 ][ 0 ] && TempVertex.color[ 0 ][ 1 ] == Vertex->color[ 0 ][ 1 ] && TempVertex.color[ 0 ][ 2 ] == Vertex->color[ 0 ][ 2 ] && TempVertex.color[ 0 ][ 3 ] == Vertex->color[ 0 ][ 3 ];
			
				if ( Compare_XYZ && Compare_Normal && Compare_ST &&
					 Compare_Lightmap && Compare_Color )
				{
					bspDrawIndexes[ NumberDrawIndexes ] = id;
					NumberDrawIndexes++;
					IsFind = qtrue;
					break;
				}
			...
			if ( !IsFind )
			{
				bspDrawIndexes[ NumberDrawIndexes ] = NumberDrawVertexes;
				bspDrawVerts[ NumberDrawVertexes ] = TempVertex;
				NumberDrawVertexes++;
				NumberDrawIndexes++;
			}
		}
	...
	/* Сохраняем новый размер массивов и освобождаем выделенную память */
	numBSPDrawVerts = NumberDrawVertexes;
	numBSPDrawIndexes = NumberDrawIndexes;

	free( Buffer_Vertexes );
	free( CountVertexInSurface );
}
...
void WriteLBSPFile( const char *filename )
{
	...
	time_t t;
	char marker[ 1024 ];
	int size, *DrawIndexes, NumberDrawVertexes = 0, NumberDrawIndexes;
	...
	*( ( int* ) ( bspHeader_t* ) header->ident ) = *( ( int* ) game->bspIdent );
	...
	/* add marker lump */
	time( &t );
	sprintf( marker, "LIFEENGINE | Q2MAP2 %s on %s)", Q3MAP_VERSION, asctime( localtime( &t ) ) );
